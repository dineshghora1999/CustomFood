        ...
        if (data[i].foodkind == "starter") {
          starter += `
                        ...
                        <div class="card-body">
                            ...
                            <div style="display: flex; justify-contents:center; align-items:center;" ><h5 class="card-title">₹ ${data[i].dataprice}</h5> <input id="ip" type="checkbox" onclick="selectDivIp(this)" data-food_type=${data[i].foodtype} data-food_kind=${data[i].foodkind} class="item" value=${data[i].name} data-price=${data[i].dataprice}></div>
...
function cart(n) {
  ...
  for (let i = 0; i < cart_div.length; i++) {
    cart_circle[i].style.color = "#396EB0";
...
function selectDivIp(ip) {
  console.log(ip.parentElement.parentElement.parentElement);

  heads = document.getElementById("heads").value;
  if (heads == 0) {
    alert("Please enter number of guests");
    window.location =  "#head_counts";
  } else {
...
function addtoplate() {
  ...
  for (
    var i = 0, i2 = 0, i3 = 0;
    i < table.length, i2 < table2.length, i3 < table3.length;
    i++, i2++, i3++
  ) {
    for (
      var l = 0, j = 0, k = 0, f = 0;
...
function show_payment_page() {
  document.getElementsByClassName("payment_container")[0].style.display =
    "block";
  document.getElementById("set_price").innerHTML = "₹ " + sum;
  var d = new Date();
  var date = d.getDate();
  ...
  document.getElementById("set_date").innerHTML = setDate;
  document.getElementById("set_delivery_date").innerHTML = "Contact Accordingly (2342-3121-87)";

      ...
      for (var i = 0; i < data.length; i++) {
        if (data[i].type == "starter") {
          starterfood += `
                    ...
                    <div class="card-body">
                        <div class="title"><span style="font-weight: bolder; font-size: larger;">${data[i].dishname}</div>
...
function increment(item) {
  ...
  if (c == 1) {
    ...
    for (
      let j = 0, k = 0, l = 0, t = 0;
      j < fname.length, k < price.length, l < quantity.length, t < total.length;
      j++, k++, l++, t++
    ) {
      let n = fname[j];
      if (m == n) {
...
function UpdateCart(fname, price, quantity, total) {
  ...
  for (let i = 0; i < table.length; i++) {
    for (
      let n = 0, q = 0, p = 0, t = 0;
      n < fname.length, q < quantity.length, p < price.length, t < total.length;
      n++, q++, p++, t++
    ) {
      ...
      butdel.onclick = function () {
        ...
        if (q1 == 1) {
          for (
            let quan = 0, nam = 0, pri = 0, tot = 0, div = 0;
            quan < quantity.length,
              nam < fname.length,
              pri < price.length,
              tot < total.length,
              div < ipdiv.length;
            quan++, nam++, pri++, tot++, div++
          ) {
...
function show_payment_page() {
  ...
  document.getElementById("set_delivery_date").innerHTML = "45 Minutes";
